...
const main = async () => {
  ...
  let hidenseek = await eventFuncs.createEvent("Hide and Seek", [], favardin.id, "2022-05-28", "12:00", "Palmer Lawn", "Hide and seek on Palmer Lawn", ["Outdoors", "Sports"]);
  let freedogs = await eventFuncs.createEvent("Free Dog Giveaway", [], calvin.id, "2022-06-01", "03:30", "Church Square Park", "Come and get a free dog, no questions asked", ["Free Stuff", "Outdoors"]);
  let painting = await eventFuncs.createEvent("Bob Ross Painting", [], phill.id, "2022-06-07", "10:15", "Bissinger", "Follow along with an episode of Bob Ross", ["Arts and Crafts", "Indoors"]);
  let racing = await eventFuncs.createEvent("Shopping Cart Races", [], ryan.id, "2022-05-25", "05:00", "ShopRite of Hoboken", "Bring your own shopping cart...", ["Sports", "Indoors", "Shopping"]);
